...
(function(root){
  ...
  function paint(g,geometry,W,event,t,p,{ellipse,line,color,skinColor,personHead}){
    const q=pose(event,geometry,W),clock=geometry.motionAge(event,'x'),skin=skinColor(event.seed),shirt=color(event.seed),scooter=event.type==='scooter';
    ...
    if(scooter){
      ...
      const push=Math.max(0,Math.sin(clock*2.3)),back=-3-push*4,lift=Math.max(0,-Math.sin(clock*2.3))*2;
      ...
      line(g,0,-8,0,-14,shirt,2.7);line(g,0,-13,-4,-10+Math.sin(clock)*.4,skin,1.1);line(g,0,-13,4,-10-Math.sin(clock)*.4,skin,1.1);
    }
